...
`translate.go` 提供了不启动 Web 服务的命令行翻译模式。该模式会读取本地文本文件，直接调用 Ollama，并在终端中流式输出思考过程和翻译结果。

### 1. 准备输入文本

将需要翻译的内容保存到项目根目录的 `text.txt` 文件中。CLI 默认把文件内容翻译为中文。

### 2. 确认默认模型可用

CLI 默认使用 `qwen3:8b`，运行前确认该模型已经下载：
...
运行后，终端会实时显示模型的思考过程和翻译结果。翻译完成后，包含模型信息、思考内容、翻译结果和性能统计的完整响应会保存到：
...
| 配置 | 默认值 | 修改位置 |
| --- | --- | --- |
| 输入文件 | `text.txt` | `translate.go` 中的 `filename` |
| 目标语言 | 中文 | 请求提示词中的目标语言 |
| 模型 | `qwen3:8b` | `translate.go` 中的 `model` |
| Ollama 接口 | `http://127.0.0.1:11434/api/chat` | `translate.go` 中的 `endpoint` |

如需翻译其他文件、切换模型或更改目标语言，请修改以上对应配置后重新运行命令。
...
在运行 `go run main.go` 的终端中按 `Ctrl+C`，服务会执行安全关闭。CLI 翻译完成后会自动退出，也可以在执行过程中按 `Ctrl+C` 中止。
